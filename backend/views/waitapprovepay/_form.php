...
<?php if (!empty(\Yii::$app->session->getFlash('msg-success'))): ?>
    <div class="alert alert-success"><i class="fa fa-check"></i> <?= \Yii::$app->session->getFlash('msg-success') ?>
    </div>
<?php endif; ?>
    <div class="row">
        <div class="col-lg-12" style="text-align: right;">
            <div class="btn btn-info" onclick="copyJob(<?= $model->id ?>)"><i class="fa fa-copy"> </i> Copy ใบงานนี้
            </div>
        </div>
    </div>
        ...
        <div class="row" style="background-color: lightblue;padding: 8px;">
            ...
            <div class="col-lg-3">
                <?= $form->field($model, 'emp_sale_id')->widget(\kartik\select2\Select2::className(), [
                    ...
                    'options' => ['id'=>'emp-sale-id','placeholder' => 'Select a  ...',],
                    'pluginOptions' => ['allowClear' => true],
                ]) ?>
            </div>
            <div class="col-lg-3">
                <?= $form->field($model, 'job_type_id')->widget(\kartik\select2\Select2::className(), [
                    ...
                    'options' => ['id' => 'job-type-id','placeholder' => 'Select a Job type ...','onchange' => 'enableinstallteam($(this));'],
                    'pluginOptions' => ['allowClear' => true],
                ]) ?>
            </div>
        </div>
        <div class="row" style="background-color: lightblue;padding: 8px;">
            ...
            <div class="col-lg-3">
                <?= $form->field($model, 'install_team_id')->widget(\kartik\select2\Select2::className(), [
                    ...
                    'options' => ['id'=>'install-team-id','placeholder' => 'Select a team ...',],
                    'pluginOptions' => ['allowClear' => true],
                ]) ?>
            </div>
            <div class="col-lg-3">
                <?= $form->field($model,'job_type_description')->textInput(['maxlength' => true,'readonly' => 'readonly','class'=>'form-control job-type-description']) ?>
            </div>
            <div class="col-lg-3">
                <?= $form->field($model, 'status')->textInput(['readonly' => 'readonly', 'style' => $model->status == 1 ? 'color:red' : 'color:green', 'value' => $model->isNewRecord ? 'Open' : \backend\helpers\JobStatus::getTypeById($model->status)]) ?>
            </div>
            <div class="col-lg-3">
<!--                --><?php //= $form->field($model, 'main_distributor_id')->widget(\kartik\select2\Select2::className(), [
        ...
        <div class="row" style="background-color: lightblue;padding: 8px;">
            <div class="col-lg-12">
                <div class="row">
                    ...
                    <div class="col-lg-2">
                        <label for="">ราคาต่อหน่วย / Unit Price</label>
                ...
                <div class="row">
                    <div class="col-lg-3">
                        ...
                        echo \kartik\select2\Select2::widget([
                            'name' => 'stock_id',
                            'data' => \yii\helpers\ArrayHelper::map(\backend\models\Warehouse::find()->where(['status' => 1])->all(), 'id', 'name'),
                            'id' => 'stock-id',
                            'options' => ['class' => 'form-control product-id', 'placeholder' => 'เลือกรายการ ...'],
                            'pluginOptions' => ['allowClear' => true]
                        ])
                        ?>
                    </div>
                ...
                <?php if ($model->isNewRecord): ?>
                    <tr>
                        ...
                        <td style="padding: 0">
                            ...
                            <input type="hidden" class="line-stock-id" name="line_stock_id[]" value="">
                        ...
                        <?php foreach ($model_line as $value): ?>
                            ...
                            <tr data-var="<?= $value->id ?>">
                                ...
                                <td style="padding: 0">
                                    ...
                                    <input type="hidden" class="line-distributor-id" name="line_distributor_id[]"
                                           value="<?= $value->distributor_id ?>">
                                    <input type="hidden" class="line-stock-id" name="line_stock_id[]"
                                           value="<?= $value->stock_type_id ?>">
                                    <input type="text" class="form-control line-total-quote-price"
                                           name="line_total_quote_price[]"
                                           style="border: none;width: 100%;box-sizing: border-box;text-align: right;"
...
?>
    ...
    <div class="form-group">
        ...
        <?php if ($model->status <= 1 || $model->isNewRecord): ?>
...
<?php ActiveForm::end(); ?>
    ...
    <form id="form-job-payment" action="<?= \yii\helpers\Url::to(['job/createpaymentnew'], true) ?>"
        ...
        <div class="col-lg-12">
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <th style="background-color: lightseagreen;color: white;text-align: right;width: 10%">หลักฐาน</th>
                    ...
                    <?php foreach ($model_payment as $value): ?>
                        ...
                        <tr data-var="<?= $value->id ?>">
                            ...
                            <td style="text-align: right;"><a
                                        href="<?= \Yii::$app->request->baseUrl . '/uploads/slip_doc/' . $value->slip_doc ?>"
                                        target="_blank"><?= $value->slip_doc != '' ? 'แสดงรูปภาพ' : 'ไม่มีรูปภาพ' ?></a>
                            </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="6" style="text-align: center;color: red">ไม่พบรายการ</td>
                    </tr>
                <?php endif; ?>
                </tbody>
                <tfoot>
        ...
        <div class="col-lg-6">
            <div class="btn-group">
                <?php if ($model->status == 1): ?>
                    <div class="btn btn-primary" onclick="submitJob();">นำส่งข้อมูล / Submit</div>
                <?php endif; ?>
                <?php if ($model->status == 2): ?>
                    <div class="btn btn-warning" onclick="paymentreviewjob();">รีวิวการชำระเงิน / Payment Review</div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <input type="hidden" class="current-submit-job-id" name="job_id" value="<?= $model->id; ?>">
...
$url_to_submit_job = \yii\helpers\Url::to(['job/submitjob'], true);
$url_to_review_payment_job = \yii\helpers\Url::to(['job/paymentreviewjob'], true);
$js = <<<JS
var selecteditem = [];
var removelist = [];
$(function(){
    calTotal();
    $("#install-team-id").prop("disabled", "disabled");
...
function createproductline(){
    ...
    var job_type_id = $("#job-type-id").val();
   
    if(job_type_id == -1 || job_type_id == null){
        alert("กรุณาเลือกประเภทงาน");
        return false;
    }
    if(job_type_id >=2 && job_type_id <= 5){
        var team_install_id = $("#install-team-id").val();
       // console.log(team_install_id);
        if(team_install_id === -1 || team_install_id == null || team_install_id == ''){
            alert("กรุณาเลือกทีมติดตั้ง");
            return false;
        }
...
function submitJob(){
        //e.preventDefault();
        //alert(url);
        var id = $(".current-submit-job-id").val();
        if(id!=null){
            swal({
                title: "ต้องการยืนยันส่งรายการนี้ใช่หรือไม่",
                text: "",
                type: "warning",
    ...
    function enableinstallteam(e){
      var id = e.val();
      //alert(id);
      if (id !=2 && id !=6) {
        $("#install-team-id").prop("disabled", "");
        $(".job-type-description").prop("readonly","readonly");
      }else if(id != 6){
        $("#install-team-id").prop("disabled", "disabled");
        $(".job-type-description").prop("readonly","readonly");
      }else if(id==6){
          $(".job-type-description").prop("readonly","");
          $("#install-team-id").prop("disabled", "disabled");
      }
